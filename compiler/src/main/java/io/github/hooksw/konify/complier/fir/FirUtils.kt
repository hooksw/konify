...
fun FirFunction.hasKonifyAnnotation(session: FirSession): Boolean =
    hasAnnotation(KonifyClassIds.View, session)
...
fun FirAnnotationContainer.hasReadOnlyKonifyAnnotation(session: FirSession): Boolean =
    hasAnnotation(KonifyClassIds.ReadOnlyView, session)
...
//    hasAnnotation(KonifyClassIds.DisallowKonifyCalls, session)

fun FirCallableSymbol<*>.isKonify(session: FirSession): Boolean =
    when (this) {
        is FirFunctionSymbol<*> ->
            hasKonifyAnnotation(session)
        is FirPropertySymbol ->
            getterSymbol?.let {
                it.hasKonifyAnnotation(session) || it.isKonifyDelegate(session)
            } ?: false
        else -> false
    }
...
fun FirCallableSymbol<*>.isReadOnlyKonify(session: FirSession): Boolean =
    when (this) {
        is FirFunctionSymbol<*> ->
            hasReadOnlyKonifyAnnotation(session)
        is FirPropertySymbol ->
            getterSymbol?.hasReadOnlyKonifyAnnotation(session) ?: false
        else -> false
    }
