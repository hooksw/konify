...
import io.github.hooksw.konify.compiler.conf.packageName
...
class KonifyInlineLambdaLocator(private val context: IrPluginContext) {
    ...
    fun preservesComposableScope(irFunction: IrFunction): Boolean =
        inlineLambdaToParameter[irFunction.symbol]?.let {
            !it.isCrossinline && !it.type.hasAnnotation(KonifyAnnotations.DisallowKonifyCalls.asSingleFqName())
        } ?: false
...
fun IrType.isSyntheticComposableFunction() =
    classOrNull?.owner?.let {
        it.name.asString().startsWith("ComposableFunction") &&
            it.packageFqName == FqName(packageName)
    } ?: false
